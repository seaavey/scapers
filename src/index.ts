import translate from "./scapers/translate"
import deepseekv1 from "./scapers/deepseek-coder-67b"
import jawdalSholat from "./scapers/jadwal-sholat"
import negro from "./scapers/negro"
import anoboy from "./scapers/anoboy"
import aio from "./scapers/aio"
import text2img from "./scapers/text2img"
import aiFluxFast from "./scapers/aiFluxFast"
import downloadCapCut from "./scapers/capcut"
import randomAnimeQuote from "./scapers/quotesAnime"

import Pterodactyl from "./utils/pterodacty"

export { anoboy, aio, translate, deepseekv1, jawdalSholat, negro, text2img, Pterodactyl, aiFluxFast, downloadCapCut, randomAnimeQuote }

import * as func from "./utils/general"

import * as sakuranovel from "./scapers/sakuranovel"
import * as bstation from "./scapers/bstation"
import * as Tiktok from "./scapers/tiktok"
import * as AniList from "./scapers/AniList"
import * as Github from "./scapers/github"
import * as Pin from "./scapers/pinterest"
import * as Animexin from "./scapers/Animexin"
import * as ytdl from "./scapers/youtube"

export { AniList, Github, Pin, Tiktok, Animexin, func, ytdl, bstation, sakuranovel }
